...
def compute_pesq(clean, processed, sr=16000):
    """
    Compute PESQ score between clean and processed audio
    
    Args:
        clean: Clean reference signal
        processed: Processed signal (either noisy or enhanced)
        sr: Sample rate
    ...
    try:
        ...
        # PESQ requires narrowband (8kHz) or wideband (16kHz)
        if sr not in [8000, 16000]:
            print(f"Resampling from {sr}Hz to 16000Hz for PESQ calculation")
        ...
        score = pesq(sr, clean, processed, mode)
        return score
...
def compute_ipd_error(clean_left, clean_right, enhanced_left, enhanced_right, sr=16000):
    """
    Compute IPD error between clean and enhanced binaural signals in degrees,
    focusing on 200-8000 Hz where IPD is perceptually relevant
    """
    # STFT parameters
    sr_ratio = sr / 16000
    n_fft = int(512 * sr_ratio)
    ...
    # Focus on 200-8000 Hz (not 200-1500 Hz)
    freqs = librosa.fft_frequencies(sr=sr, n_fft=n_fft)
    f_min = 200  # Hz
    f_max = 1500  # Hz - matching paper frequency range
    freq_mask = np.logical_and(freqs >= f_min, freqs <= f_max)
    
    # Ensure correct broadcasting
    freq_mask = freq_mask[:, None]  # Shape [F, 1]
    combined_mask = mask & freq_mask  # Broadcasting should work correctly
...
def safe_mbstoi(clean_left, clean_right, enhanced_left, enhanced_right, sr=16000):
    """Compute MBSTOI with proper padding for signals"""
    import math
    import numpy as np
    
    # Import your existing MBSTOI implementation
    from MBSTOI.mbstoi import mbstoi
    
    # Ensure all signals have the same length
    min_len = min(len(clean_left), len(clean_right), len(enhanced_left), len(enhanced_right))
    
    # Make target length at least 2 seconds and divisible by 256
    target_len = max(2 * sr, ((min_len + 255) // 256) * 256)
    
    # Helper function to trim and pad signals
    ...
    try:
        # Use your existing MBSTOI implementation
        # Adjust the parameters based on your function signature
        score = mbstoi(clean_left, clean_right, enhanced_left, enhanced_right, gridcoarseness=1)
        
        # Validate result - if invalid, return None instead of fallback
...
def evaluate_at_paper_snrs(model_checkpoint, test_dir, clean_dir, output_dir, batch_size=8, limit_pairs=None):
    ...
    with open(os.path.join(output_dir, "paper_style_results.csv"), 'w') as f:
        f.write("SNR,MBSTOI,ΔSegSNR,ΔPESQ,L_ILD,L_IPD\n")
        ...
        for snr in paper_snrs:
            if snr in results_by_snr:
                ...
                f.write(f"{snr},{result.get('Average MBSTOI', 'N/A'):.4f}," +
                        ...
                        f"{result.get('Average ΔPESQ', 'N/A'):.2f}," +
    ...
    avg_pesq = np.mean([r.get('Average ΔPESQ', 0) for r in results_by_snr.values()])
    ...
    print(f"Average ΔPESQ: {avg_pesq:.2f}")
    ...
    with open(os.path.join(output_dir, "paper_style_averages.txt"), 'w') as f:
        ...
        f.write(f"Average ΔPESQ: {avg_pesq:.2f}\n")
...
def evaluate_model(model_checkpoint, test_dataset_path, clean_dataset_path, output_dir, 
    ...
    results = {
        ...
        'segSNR_L': [],
        'segSNR_R': [],
        'ΔPESQ_L': [],   # Added ΔPESQ for left channel
        'ΔPESQ_R': [],   # Added ΔPESQ for right channel
        'MBSTOI': [],
        'ILD_error': [],
    ...
    def calc_metrics(clean_data, noisy_data, enhanced_data, sr):
        ...
        pesq_noisy_L = compute_pesq(clean_data[:, 0], noisy_data[:, 0], sr=sr)
        pesq_enhanced_L = compute_pesq(clean_data[:, 0], enhanced_data[0], sr=sr)
        delta_pesq_L = pesq_enhanced_L - pesq_noisy_L
        
        pesq_noisy_R = compute_pesq(clean_data[:, 1], noisy_data[:, 1], sr=sr)
        pesq_enhanced_R = compute_pesq(clean_data[:, 1], enhanced_data[1], sr=sr)
        delta_pesq_R = pesq_enhanced_R - pesq_noisy_R
        ...
        return segSNR_L, segSNR_R, delta_pesq_L, delta_pesq_R, mbstoi_score, ild_error, ipd_error_unitless, ipd_error_degrees
    ...
    for batch_idx in tqdm(range(total_batches), desc=f"Processing batches (size={batch_size})", total=total_batches):
        ...
        for i in range(len(valid_pairs)):
            try:
                ...
                segSNR_L, segSNR_R, delta_pesq_L, delta_pesq_R, mbstoi_score, ild_error, ipd_error_unitless, ipd_error_degrees = calc_metrics(
                ...
                if mbstoi_score is not None:
                    ...
                    results['ΔPESQ_L'].append(delta_pesq_L)
                    results['ΔPESQ_R'].append(delta_pesq_R)
                    ...
                    if batch_idx == 0 and i < 3:
                        ...
                        print(f"- ΔSegSNR: L={segSNR_L:.2f} dB, R={segSNR_R:.2f} dB")
                        print(f"- ΔPESQ: L={delta_pesq_L:.2f}, R={delta_pesq_R:.2f}")
    ...
    average_results = {
        ...
        'Average ΔPESQ (L)': results_df['ΔPESQ_L'].mean(),
        'Average ΔPESQ (R)': results_df['ΔPESQ_R'].mean(),
        'Average ΔPESQ': (results_df['ΔPESQ_L'].mean() + results_df['ΔPESQ_R'].mean()) / 2,
...
def generate_paper_tables(vctk_results, timit_results, output_dir):
    ...
    with open(os.path.join(output_dir, "table1_anechoic_results.csv"), 'w') as f:
        f.write("Input SNR,MBSTOI,ΔSegSNR,ΔPESQ,L_ILD (dB),L_IPD\n")
        
        for snr in snr_values:
            if snr in vctk_results and vctk_results[snr]:
                result = vctk_results[snr]
                f.write(f"{int(snr)},{result.get('Average MBSTOI', 'N/A'):.2f},{result.get('Average SegSNR Improvement', 'N/A'):.1f},{result.get('Average ΔPESQ', 'N/A'):.2f},{result.get('Average ILD Error (dB)', 'N/A'):.2f},{result.get('Average IPD Error (paper)', 'N/A'):.2f}\n")

    # Table 2: Unmatched condition (TIMIT) results if available
    if timit_results:
        with open(os.path.join(output_dir, "table2_timit_results.csv"), 'w') as f:
            f.write("Input SNR,MBSTOI,ΔSegSNR,ΔPESQ,L_ILD (dB),L_IPD (degrees)\n")
            
            for snr in snr_values:
                if snr in timit_results and timit_results[snr]:
                    result = timit_results[snr]
                    f.write(f"{snr},{result.get('Average MBSTOI', 'N/A'):.2f},{result.get('Average SegSNR Improvement', 'N/A'):.1f},{result.get('Average ΔPESQ', 'N/A'):.2f},{result.get('Average ILD Error (dB)', 'N/A'):.2f},{result.get('Average IPD Error (degrees)', 'N/A'):.1f}\n")
    ...
    with open(os.path.join(output_dir, "comparison_with_paper.csv"), 'w') as f:
        ...
        for snr in snr_values:
            if snr in vctk_results and vctk_results[snr] and snr in paper_results:
                result = vctk_results[snr]
